...
CIPHER_IMG = pygame.image.load(os.path.join("cipher_images", "encrypted4.png"))  # Change this to the image your trying to decrypt
...
class Encoder:
    ...
    def decrypt_color_squares(self):
        for x in range(0, 900, 50):
            rgb = (WIN.get_at((x, 100))[0], WIN.get_at((x, 100))[1], WIN.get_at((x, 100))[2])
            for i, color in enumerate(self.color_key.values()):
                if color == rgb:
                    self.cipher_text += list(self.color_key.keys())[i][0]
...
def main(desicion, text, cipher_text, key, color_key):
    ...
    while run:
        ...
        #pygame.image.save(WIN, "encrypted5.png")   # Uncomment this to save you encrypted image and change name of file
